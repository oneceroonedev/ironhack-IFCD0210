...
function suitableForScholarship() {
    ...
    if (age > 30) {
        let largeFamily = removeAccents(prompt("¿Perteneces a una familia numerosa? (Escribe si o no)").toLowerCase()) === "si";
        if (largeFamily) {
    ...
    if (age >= 12 && age <= 18) {
        let geniusDeveloper = removeAccents(prompt("¿Eres un genio de la programación? (Escribe si o no)").toLowerCase()) === "si";
        if (geniusDeveloper) {
